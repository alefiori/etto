...
Removing the font dependency: the tagline is outlined from the Figtree subset
the app ships (public/fonts/), so it matches the first painted frame. The
wordmark above it is not type at all — it is the geometric Etto mark
(design/logo-concepts/wordmark-etto.svg), drawn here as plain shapes, so there
is nothing to resolve.

    pip install fonttools brotli
    python3 scripts/build-splash.py

Re-run it after changing the wordmark shapes, the tagline, or the Figtree
subset. The strings live in WORDMARK / TAGLINE below, so they stay greppable
even though the output has no text in it.
"""
...
# The Etto wordmark as monolinear geometric letterforms — not a typeface, so
# nothing here resolves to a font. Centred on x=0 with the baseline at y=0, to
# drop into the same slot the outlined type used. Each t has its own crossbar
# with a gap between (one shared bar made "tt" read as "H"); the o opens at the
# bottom. Mirrors design/logo-concepts/wordmark-etto.svg, translated there by
# (-253.5, -165) to put the origin on the baseline centre. `{fill}` is the
# scheme's wordmark colour, substituted in build().
WORDMARK_SHAPES = """      <rect x="-229.5" y="-130" width="24" height="130" rx="6"/>
      ...
      <circle cx="154.5" cy="-65" r="63" fill="none" stroke="{fill}" stroke-width="24" stroke-linecap="round" stroke-dasharray="330 66" stroke-dashoffset="-132"/>"""
...
TEMPLATE = """<svg width="2732" height="2732" viewBox="0 0 2732 2732" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Launch screen, {scheme}. GENERATED by scripts/build-splash.py — edit that,
       not this. The wordmark is the geometric Etto mark drawn as shapes; the
       line under it is outlined from the Figtree subset — because this file is
       rasterized outside a browser and no `font-family` here would resolve.
...
{icon}
  ...
  <!-- "{wordmark_text}" — geometric wordmark, baseline centred at (1366, 1596). -->
  <g transform="translate(1366 1596) scale({wordmark_scale})" fill="{wordmark}">
{wordmark_shapes}
  </g>
...
def build(spec: dict) -> str:
    return TEMPLATE.format(
        ...
        # 0.82 lands the geometric wordmark's cap height near the outlined
        # Instrument Serif it replaces (its heavier forms read larger at parity).
        wordmark_scale=0.82,
        wordmark_shapes=WORDMARK_SHAPES.format(fill=spec["wordmark"]),
