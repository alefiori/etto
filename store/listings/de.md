# Store listing — Deutsch (de)

Zeichenlimits in Klammern: jeweils das strengere der beiden Stores. Ein zu
langes Feld wird beim Upload abgelehnt, nicht erst im Review.

**Dieser Text beschreibt nur, was die App heute tatsächlich kann.** Siehe en.md.

---

## App name [30]

MacroTrack

## Subtitle — App Store [30]

Deine Makros, jeden Tag

## Short description — Play [80]

Kohlenhydrate, Eiweiß und Fett verfolgen — mit einem Ziel für jeden Wochentag.

## Promotional text — App Store [170]

Für jeden Wochentag eigene Ziele, Erfassung per Barcode-Scan und Ringe, die sich
füllen. Kostenlos, ohne Konto zum Loslegen.

## Keywords — App Store [100, comma-separated, no spaces]

makro,makros,kalorien,zähler,ernährung,eiweiß,protein,kohlenhydrate,diät,tagebuch,barcode,gewicht

## Full description [4000]

MacroTrack ist ein Makronährstoff-Tracker für alle, die schon wissen, was sie
essen wollen, und ihre Zahlen einfach treffen möchten, ohne sich mit einer App
zu streiten.

Öffnen und loslegen. Keine Registrierungspflicht, kein Einstiegsfragebogen, kein
Konto — bis du deine Daten auf einem zweiten Gerät haben willst.

**Ziele, die zu deiner Woche passen**
Die meisten Apps geben ein einziges Tagesziel. MacroTrack gibt sieben: Stelle
Kohlenhydrate, Eiweiß und Fett für jeden Wochentag ein, damit ein Trainingstag
und ein Ruhetag nicht dieselbe Zahl sind. Und wenn doch, kopierst du einen Tag
auf die ganze Woche.

**Erfassen, das nicht bremst**
Suche in deinen eigenen Lebensmitteln, in denen anderer MacroTrack-Nutzer, in
nationalen Nährwerttabellen und in zwei kostenlosen Datenbanken — Open Food Facts
und USDA FoodData Central — in einer einzigen Liste, jedes Ergebnis mit seiner
Herkunft gekennzeichnet. Oder scanne den Barcode auf der Packung und überspringe die Suche
ganz.

Fasse Lebensmittel zu Mahlzeiten zusammen, die du selbst benennst. Umbenennen,
umsortieren, hinzufügen, entfernen — dein Tag muss nicht aus Frühstück,
Mittagessen und Abendessen bestehen, wenn er das nicht tut.

**Der Tag auf einen Blick**
Drei Ringe zeigen Verbrauchtes gegen Ziel, darunter das Verbleibende, dazu eine
Kalorienbilanz, die beim Erfassen mitwächst. Kopiere einen ganzen Tag auf ein
anderes Datum oder eine einzelne Mahlzeit in einen beliebigen Slot an einem
beliebigen Tag — praktisch, wenn du fünf Tage die Woche dasselbe frühstückst.

**Gewicht, ohne das Rauschen**
Ein Wiegen pro Tag, als Rohwerte gepunktet mit einer geglätteten Linie
hindurch — so sieht eine nächtliche Wasserschwankung nicht nach Zunahme aus. Die
Wochenrate wird robust geschätzt, ein schlechter Morgen verschiebt sie also
nicht.

**Wasser, ohne zweite App**
Gläser und Flaschen mit einem Tipp hinzufügen, gegen ein Tagesziel, das sich aus
deinem Körpergewicht ergibt, solange du keines festlegst.

**Deine Lebensmittel — und die aller anderen**
Lege eigene Lebensmittel mit Makros pro Portion und laufender Kalorienrechnung
an. Teile eines mit der Community, und andere finden und verwenden es — oder
behalte es privat, was die Voreinstellung ist.

**Bleibt deins**
Teile eine Mahlzeit oder einen ganzen Tag als einfachen Text, direkt in jede
Messenger-App. Lösche dein Konto samt allem darin jederzeit aus der App heraus.

**Funktioniert überall**
Verfügbar auf Englisch, Italienisch, Französisch, Spanisch, Deutsch,
Portugiesisch und Niederländisch — folgt deiner Gerätesprache, bis du eine
wählst. Helles und dunkles Design, das ebenfalls dem Gerät folgt. Ein echtes
Tablet-Layout statt eines vergrößerten Handys.

**MacroTrack Pro (optional)**
Alles oben ist kostenlos. Pro ergänzt adaptive Ziele: Statt eine Formel mit
einer geschätzten Aktivität zu multiplizieren, ermittelt es deinen tatsächlichen
Verbrauch aus dem Vergleich zwischen dem, was du erfasst hast, und dem, wie sich
dein Gewicht wirklich entwickelt hat — und sagt dir, wenn die Daten für eine
Antwort noch nicht reichen, statt eine zu erfinden.

Pro gibt es im Monats- oder Jahresabo oder als einmaligen Lifetime-Kauf. Abos
verlängern sich automatisch bis zur Kündigung; verwalten oder kündigen kannst du
sie in den Kontoeinstellungen deines Stores.

---

MacroTrack ist ein Tracking-Werkzeug und keine medizinische Beratung. Ziele und
Schätzungen werden aus deinen Eingaben berechnet und können falsch sein. Sprich
mit einer Ärztin oder einer Ernährungsberatung, bevor du deine Ernährung
umstellst.

Nährwertdaten von Open Food Facts (ODbL), USDA FoodData Central (gemeinfrei),
ANSES-Ciqual (Licence Ouverte), CoFID (Open Government Licence v3.0) und CREA.
